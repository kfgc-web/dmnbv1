...
Última atualização: 26/09/2026 (15 objetivos novos no Clássico; commits em nome de Kauã).
...
- **Autoria dos commits:** em nome de Kauã (`git config user.name "kfgc-web"` e `user.email "kauafecamargo@gmail.com"`, a configurar no início de cada sessão), com o Claude como coautor (linha `Co-Authored-By` no fim da mensagem). Decisão de Kauã (26.9.2026). Os commits antigos ficam como estão (reescrever o histórico não vale o risco).
- Se mudar regra, objetivo, carta, território ou o mapa, avisar o Kauã que os livros e as cartas em `livros/` ficaram desatualizados.
...
| `livros/` | Livros de regras (digital e de mesa), catálogo das cartas e arquivos de impressão (cartas e tabuleiros). Os PDFs são gerados fora do repositório a partir dos `.md` em `livros/fontes/` e do desenho do jogo (`cartas.js`, `desenho.js`). |
